...
description: "地下水流動の基本である「ダルシーの法則」の理論と、FloPy（MODFLOW 6）を用いた1次元モデルによる再現方法を解説します。"
image: "image.png"
author: "DeepFlow"
execute:
  enabled: false
...
地下水モデリングを学ぶ上で、最初に理解すべき最も重要な原則が**「ダルシーの法則（Darcy's Law）」**です。
1856年、フランスの技術者アンリ・ダルシーは、砂を詰めた円筒に水を流す実験を行い、「水を通しやすさ（透水係数）」と「水位の差（動水勾配）」が水の流れる量に比例することを発見しました。
...
また、断面積 $A$ で割った「比流量（Darcy流速）」$q$（m/day）は次のように表されます。
...
## 5. 【重要】CHDはセル中心に水頭を与える

コードを走らせる前に、**数値モデルと手計算の比較で必ずつまずくポイント**を理解しておこう。

### CHDパッケージの仕組み

MODFLOW 6の`CHD`（Constant Head）パッケージは、「境界面」ではなく**セルそのものに固定水頭を与える**。つまり、水頭が定義されるのはセルの物理的な中心点だ。
...
このモデルの「実際のカラム長」は 100 m だが、**水頭の境界条件が与えられているのは x = 5 m と x = 95 m の間**、すなわち 90 m の区間だ。
...
セル数を10倍にするほど誤差は約1/10に減る。これが空間収束性（spatial convergence）だ。
...
数値モデルと解析解を比較するとき、**「境界条件がどこに置かれているか」を常に意識すること**。これはMODFLOWに限らず、有限差分法・有限要素法すべてに共通する重要な概念だ。次の記事では、より複雑な2次元モデルや井戸揚水を追加していく。
:::
