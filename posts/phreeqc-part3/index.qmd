...
この2つは同じ化学組成（Ca・S・O）でできているが、**温度によってどちらが安定かが逆転する**という興味深い性質を持つ。これを計算で確かめていこう。
...
GUI操作の結果として生成される完全なコードは以下の通りである。直接貼り付けて実行することもできる。
...
<div style="background:#F8F8F8; border:1px solid #E5E7EB; border-radius:8px; padding:1.2rem 1.5rem; margin:1.5rem 0;">
  <div style="font-weight:500; color:#111827; margin-bottom:1rem;">SI の温度依存性（概念グラフ）</div>

  <div style="position:relative; height:180px; margin-bottom:0.5rem;">
    <!-- Y軸ゼロライン -->
    <div style="position:absolute; top:50%; left:0; right:0; border-top:1px dashed #9CA3AF;"></div>
    <div style="position:absolute; top:46%; right:4px; font-size:11px; color:#9CA3AF;">SI = 0</div>

    <!-- Gypsum の曲線（SVG） -->
    <svg style="position:absolute; top:0; left:0; width:100%; height:100%;" viewBox="0 0 400 180" preserveAspectRatio="none">
      <!-- Gypsum：左から右へゆっくり下降（SI がマイナスに） -->
      <polyline points="10,88 60,92 110,97 160,103 210,110 260,118 310,127 360,138 390,148"
                fill="none" stroke="#3B82F6" stroke-width="2.5"/>
      <!-- Anhydrite：左から右へ上昇（SIが0に近づき、やがてプラスに） -->
      <polyline points="10,118 60,112 110,105 160,98 210,91 260,84 310,78 360,72 390,68"
                fill="none" stroke="#D97706" stroke-width="2.5"/>
      <!-- 交点あたりの縦線 -->
      <line x1="215" y1="20" x2="215" y2="170" stroke="#E5E7EB" stroke-width="1" stroke-dasharray="3,3"/>
    </svg>

    <!-- 凡例 -->
    <div style="position:absolute; top:8px; left:12px; display:flex; flex-direction:column; gap:4px;">
      <div style="display:flex; align-items:center; gap:6px; font-size:12px;">
        <div style="width:20px; height:3px; background:#3B82F6; border-radius:2px;"></div>
        <span style="color:#1E40AF;">Gypsum（SI低下）</span>
      </div>
      <div style="display:flex; align-items:center; gap:6px; font-size:12px;">
        <div style="width:20px; height:3px; background:#D97706; border-radius:2px;"></div>
        <span style="color:#92400E;">Anhydrite（SI上昇→0へ）</span>
      </div>
    </div>
  </div>
...
低温では、水分子を結晶格子に取り込んだ石膏（CaSO₄・2H₂O）の方が安定である。GypsumのSIはゼロに近く（平衡付近）、AnhydriteのSIはマイナス（不飽和・溶解方向）となる。

### ② 55℃付近で逆転

温度が上昇するにつれて、AnhydriteのSIが上昇してゼロに近づく。55℃付近でAnhydriteのSI = 0 となり、それ以上の温度ではAnhydriteが過飽和（沈殿方向）になる。
...
この相転移は現実の地質現象にも対応している。堆積盆地の深部（温度が高い）では硬石膏が、浅部（温度が低い）では石膏が卓越する。またCCSプロジェクトでCO₂を高温の地層に圧入する際、CO₂による水の酸性化 → 石膏の溶解 → 硬石膏の沈殿、というシーケンスが起こりうる。
:::
...
土壌中のCO₂分圧（$P_{CO_2} = 10^{-1.5}$ atm）で飽和した水がカルサイトと反応するとき、系がCO₂に対して開いているか閉じているかで最終的なpHがどう変わるか ── を追う。地下水の水質形成を理解するうえで最も重要な反応の一つである。
...
- [#4 カルサイト−CO₂水反応](../phreeqc-part4/)  ← 近日公開
