...
- **Speciation**（[#2](../phreeqc-part2/)）：1点の溶液組成
- **EQUILIBRIUM_PHASES**（[#3](../phreeqc-part3/)〜[#5](../phreeqc-part5/)）：1点での鉱物平衡
- **REACTION**（[#11](../phreeqc-part11/)）：時間方向への変化
...
本回で学んだ移流分散（＝反応輸送）の考え方は、[地下水科学入門 #15 — ケイ酸塩風化と地下水の滞留時間](/posts/groundwater/groundwater-sci15/) の結論で鍵になる。混合が主体の火山湧水系では、溶存シリカの滞留時間依存を平衡計算だけでは説明できず、まさに `TRANSPORT` が扱う移流・分散・混合を連立させた反応輸送モデルが必要になる——その理由を論じている。
:::
