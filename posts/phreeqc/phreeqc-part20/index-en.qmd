...
Writing $K$ as "products ÷ reactants," the denominator terms $a(\mathrm{SiO_2})=1$ (pure solid) and $a(\mathrm{H_2O})\approx 1$ (water) vanish, leaving:

$$ K = a(\mathrm{H_4SiO_4}) = 10^{-3.734} \approx 1.8 \times 10^{-4} $$

That is "the solubility of quartz." A log K value carries a concrete equilibrium number behind it.
...
The denominator now contains $a(\mathrm{H^+})$ — this is the pH lever. Writing out the saturation index (SI) makes the role of pH explicit:

$$ \mathrm{SI} = \log a(\mathrm{Ca^{2+}}) + \log a(\mathrm{HCO_3^-}) + \mathrm{pH} - 1.847 $$

(using $-\log a(\mathrm{H^+}) = \mathrm{pH}$). So pH enters SI with a coefficient of +1: **raise pH by 1 and SI rises by 1** (i.e. ten-fold more supersaturated). This is the real mechanism behind "calcite precipitates as pH rises."
...
Forming $(1) - (2)$, the $2\mathrm{Al^{3+}}$ and $2\mathrm{H_4SiO_4}$ cancel on both sides:

$$ \text{Anorthite} + 2\mathrm{H^+} + \mathrm{H_2O} = \text{Kaolinite} + \mathrm{Ca^{2+}} $$
$$ \log K = 24.235 - 6.483 = +17.75 $$

log K = +17.75, i.e. $\Delta G^\circ(25℃) \approx -5.71 \times 17.75 \approx -101$ kJ/mol — a steep downhill. The Al in anorthite drops happily into kaolinite.
...
Taking $(1)$ as is, $(2)$ reversed, $(3)$ reversed, $(4)$ as is, and summing, the $\mathrm{Al^{3+}}$, $\mathrm{H_4SiO_4}$, $\mathrm{Ca^{2+}}$, $\mathrm{HCO_3^-}$ and $\mathrm{H^+}$ all cancel:

$$ \text{Anorthite} + \mathrm{CO_2} + 2\mathrm{H_2O} = \text{Calcite} + \text{Kaolinite} $$
$$ \log K = 24.235 - 6.483 - 1.847 - 7.821 = +8.08 $$

$\Delta G^\circ(25℃) \approx -46$ kJ/mol, downhill. The primary mineral anorthite fixes CO2 as calcite, and the leftover Al becomes kaolinite. On paper, we can see this is a **thermodynamically certain** reaction.
...
Forming $(1) - (2) - (3)$, the H+, HCO3-, Ca2+ and Mg2+ all cancel:
...
So the supply of Ca, Al and Si does not decide between them — the activity of water does. Building the dehydration reaction from the Thermoddem dissolution reactions (subtracting wairakite's dissolution from laumontite's):

$$ \text{Laumontite} = \text{Wairakite} + 2\mathrm{H_2O} $$

For this reaction $K = a(\mathrm{H_2O})^2$ (solids are 1). Hence the threshold water activity at which the two balance is $a(\mathrm{H_2O}) = 10^{\log K / 2}$. Computing the dehydration log K at each temperature in PHREEQC:
...
![Temperature dependence of the Laumontite ⇌ Wairakite dehydration log K and the threshold a(H2O). At low temperature, dilute water (a≈1) always favors laumontite. Where the threshold reaches 1, near 235 °C, wairakite appears even in pure water.](fig-zeolite-temp.png){width=85% .lightbox}

How to read it: if $a(\mathrm{H_2O})$ is above the threshold, the more hydrous laumontite is stable; below it, the dehydrated wairakite is stable. At low temperature, ordinary dilute groundwater ($a(\mathrm{H_2O}) \approx 1$) is always above the threshold, so laumontite wins. To get wairakite you must either (a) lower the water activity (high salinity, evaporation) or (b) raise the temperature.

As temperature rises the dehydration log K increases, and **at about 235 °C the threshold reaches 1, so wairakite finally appears even in pure water**. This nearly matches the phase boundary from Liou (1971) [@liou1971] (about 230 °C). The difficulty of handling zeolites in low-temperature CarbFix systems has this delicate tug-of-war over water activity behind it.
